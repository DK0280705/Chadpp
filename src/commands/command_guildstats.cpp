...
void Command_guildstats::call(const Input& input) const
{
    ...
    for (pqxx::result::size_type i = 0; i < res.size(); i++) {
        ...
        active_members += fmt::vformat(_(gl, COMMAND_GUILDSTATS_ACT_MEM_FORMAT),
    ...
    dpp::embed e  = dpp::embed()
        ...
        .add_field(_(gl, INFORMATION),
                   fmt::vformat(_(gl, COMMAND_GUILDSTATS_INFO_FORMAT),
                               fmt::make_format_args(
                                    g->owner_id, g->members.size(),
                                    g->channels.size(),
                                    bool_string(g->is_verified()),
                                    bool_string(g->is_discoverable()),
                                    _(gl, g->nsfw_level + (int)NSFW_LEVEL_DEFAULT))))
        .add_field(_(gl, COMMAND_GUILDSTATS_INFO_TOP10), active_members);
    input.reply(e);
}