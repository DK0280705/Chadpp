...
void Command_record::call(const Input& input) const
{
    ...
    if (sub == 0) {
        ...
        for (const auto& ch_id : g->channels) {
            ...
            const auto vc_members = ch->get_voice_members();

            const auto vsi = vc_members.find(input->author.id);
            if (vsi != vc_members.end()) {
                input.reply(_(input->lang_id, COMMAND_RECORD_START));
        ...
        input.reply(_(input->lang_id, COMMAND_RECORD_USR_NOT_IN_VC));
    } else if (sub == 1) {
        if (const dpp::voiceconn* vc = input->client->get_voice(input->guild_id)) {
            dpp::channel* ch = dpp::find_channel(vc->channel_id);

            const auto vc_members = ch->get_voice_members();

            if (vc_members.find(input->author.id) == vc_members.end())
                return input.reply(_(input->lang_id, COMMAND_RECORD_USR_DIFF_VC));
            ...
            if (lame_init_params(lame) < 0)
                return input.reply(_(input->lang_id, COMMAND_RECORD_ERR_PROC_AUDIO));
            ...
            while (pcm.good()) {
                ...
                const int read = pcm.gcount() / 4; // 2*sizeof(int16_t)

                const int write = (read == 0)
            ...
            dpp::message m = dpp::message(input->channel_id, _(input->lang_id, COMMAND_RECORD_STOP))
        ...
        } else input.reply(_(input->lang_id, COMMAND_RECORD_DISCONNECTED));
    }
}
